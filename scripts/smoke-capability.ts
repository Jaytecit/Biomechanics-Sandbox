...
const base = structuredClone(CREATURE_TEMPLATES.find(item => item.nodes.some(node => node.isMotorWheel)) ?? CREATURE_TEMPLATES[0]);
...
const noAero = structuredClone(base);
...
const noFlex = structuredClone(base);
for (const muscle of noFlex.muscles) { muscle.minLength = muscle.originalLength; muscle.maxLength = muscle.originalLength; muscle.strength = 1; }
